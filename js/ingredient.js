...
async function getFoodByIngredient() {
  ...
  if (res.ok) {
    ...
    let first30Res = data.meals.slice(0, 20);
    displayIngredients(first30Res);
  }
}
...
function displayIngredients(meals) {
  ...
  for (let i = 0; i < meals.length; i++) {
    let desc = meals[i].strDescription
      ? meals[i].strDescription.split(" ").slice(0, 20).join(" ") + "..."
      : "";
    cartona += `
      <div class="col-sm-12 col-md-3 col-lg-3 d-flex flex-column align-items-center pt-2">
        <i class="fa-solid fa-drumstick-bite fa-4x text-white"></i>
        <h3 class="ingredient-name text-center" data-ingredient="${meals[i].strIngredient}">${meals[i].strIngredient}</h3>
  ...
  $(".ingred").html(cartona);
  $(".ingred").show();
  $(".ingredientMeals").hide();
}

// Event: Click on Ingredient
$(".ingred").on("click", ".ingredient-name", async function () {
  ...
  if (res.ok) {
    ...
    $(".ingred").hide();
    $(".ingredientMeals").show();
  }
});

// Display Meals filtered by Ingredient
function displayIngredientMeals(meals) {
  ...
  $(".ingredientMeals").html(cartona);
}



$(".ingredientMeals").on("click", ".meal-box", async function () {
  $(".loading").fadeIn(500);
